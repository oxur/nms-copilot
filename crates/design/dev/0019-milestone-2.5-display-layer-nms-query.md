# Milestone 2.5 -- Display Layer (nms-query)

Pretty table formatters for query results. Renders portal glyphs as emoji, formats distances with K/M suffixes, and produces aligned terminal output.
...
No new dependencies -- uses `nms-core` (glyph emoji) already available.

---

## New File: `crates/nms-query/src/display.rs`
...
///
/// Uses `nms_core::glyph::Glyph::from_hex_char()` and `emoji()`.
pub fn hex_to_emoji(hex: &str) -> String {
    use nms_core::glyph::Glyph;
    hex.chars()
        .filter_map(|ch| {
            let idx = ch.to_digit(16)? as u8;
            Some(Glyph::from_index(idx).emoji().to_string())
        })
        .collect()
}
...
pub fn format_show_system(result: &ShowSystemResult) -> String {
    ...
    out.push_str(&format!("  Portal Glyphs:   {}\n", hex_to_emoji(&result.portal_hex)));
...
pub fn format_stats(result: &StatsResult) -> String {
    ...
    out.push_str(&format!("  Named Systems:   {}\n", result.named_system_count));
    out.push_str(&format!("  Named Planets:   {}\n", result.named_planet_count));
...
### Update `crates/nms-query/src/lib.rs`
...
pub use display::{format_distance, format_find_results, format_show_result, format_stats, hex_to_emoji};
```
...
### File: `crates/nms-query/src/display.rs` (inline tests at bottom)
...
mod tests {
    ...
    #[test]
    fn format_find_results_empty() {
        let output = format_find_results(&[]);
        assert!(output.contains("No results found"));
    }

    #[test]
    fn format_find_results_with_data() {
        ...
        let results = vec![FindResult {
            ...
            system: System::new(addr, Some("Sol".into()), Some("Explorer".into()), None, vec![]),
    ...
    #[test]
    fn format_find_results_infested_marker() {
...
1. **`hex_to_emoji` uses `Glyph::from_index`** -- each hex digit (0-F) maps to a glyph index. The `Glyph` type in nms-core provides `emoji()` which returns the emoji string. Verify the exact API name matches what's in `nms_core::glyph`.

2. **No ANSI colors** -- color support is deferred to Phase 7 polish. The formatters produce plain text only.

3. **Infested marker** -- infested planets show a `*` after the biome name (e.g., "Toxic*"). Simple and clear.

4. **`truncate` works on bytes** -- this is fine for ASCII names, but NMS names are typically ASCII. If UTF-8 truncation matters, switch to `char_indices`.

5. **Column widths are fixed** -- chosen to fit an 80-column terminal. If names are longer, they get truncated with "...". This is a pragmatic default; the REPL can use dynamic widths later.

6. **Distance formatting** -- uses integer K for thousands and one decimal M for millions. This matches the README examples.
