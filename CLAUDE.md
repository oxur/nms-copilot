...
**Status:** Phases 1-2 complete. Core types (`nms-core`), binary save parser (`nms-save`), galaxy model (`nms-graph`), query engine (`nms-query`), and CLI (`nms-cli`) with `info`, `convert`, `find`, `show`, and `stats` commands are implemented and tested (322 tests).
