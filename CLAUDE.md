...
**Status:** Phases 1-3 complete. Core types (`nms-core`), binary save parser (`nms-save`), galaxy model (`nms-graph`), query engine (`nms-query`), CLI (`nms-cli`) with `info`, `convert`, `find`, `show`, and `stats` commands, interactive REPL (`nms-copilot`) with session state, tab completion, rkyv cache, and TOML config are implemented and tested (421 tests).
